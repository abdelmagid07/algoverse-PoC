...
5. **Step-boundary convention.** One step = one `ai` (assistant) turn; its step
   ...
   trend.
6. **Observation truncation.** To fit the T4 token budget, each tool observation
   (and the issue text) is head-truncated to a fixed token cap while `ai`
   reasoning/action turns are kept whole. This alters the context the model reads;
   trajectories still over budget after truncation are dropped (and logged), not
   silently clipped mid-sequence.
